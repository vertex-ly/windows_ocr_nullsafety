...
bool TNSOCR::TryLoadDLL(wchar_t *DllFileName)
{
	...
	if (!DllFileName || !DllFileName[0])
		wcscpy(fn, L"NSOCR.dll"); // old style: wcscpy(fn, L"NSOCR.dll");
	else
		wcscpy(fn, DllFileName); // old style: wcscpy(fn, DllFileName);
...
{
	if (!loaded || !p_Engine_InitializeAdvanced)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Engine_Uninitialize)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Engine_SetDataDirectory)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Engine_SetLicenseKey)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Cfg_LoadOptionsFromString)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Cfg_SaveOptionsToString)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Img_LoadFromMemory)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Img_DeleteAllBlocks)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Img_GetScaleFactor)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Img_CalcPointPosition)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Img_CopyCurrentPage)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_IsWordInDictionary)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetWordFontColor)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetWordFontSize)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetWordFontStyle)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetBackgroundColor)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetBarcodeText)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetBarcodeRect)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Blk_GetBarcodeType)
		return ERROR_DLLNOTLOADED;
...
{
	if (!loaded || !p_Svr_SetDocumentInfo)
		return ERROR_DLLNOTLOADED;
